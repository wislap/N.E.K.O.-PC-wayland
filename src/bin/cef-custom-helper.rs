...
use neko_pc_wayland::cef::{
    CefCBridge, CefCBridgeConfig, CefLifecycleEvent, clear_event_callback, install_event_callback,
    run_raw_input_loop_cbridge, try_run_c_subprocess,
};
#[cfg(feature = "cef_osr")]
...
fn main() {
    if let Err(err) = run() {
        eprintln!("cef-custom-helper failed: {err:#}");
        std::process::exit(1);
    }
}
    ...
    #[cfg(feature = "cef_osr")]
    {
        ...
        if let Some(runtime_dir) = discover_runtime_dir()? {
            std::env::set_current_dir(&runtime_dir).with_context(|| {
                format!(
                    "failed to switch cef-custom-helper working directory to {}",
                    runtime_dir.display()
                )
            })?;
            eprintln!(
                "cef-custom-helper switched working directory to {}",
                runtime_dir.display()
            );
        }

        let mut host_config = RawHostConfig::probe();
        host_config.title = "neko-cef-custom-helper".to_string();
        host_config.app_id = "moe.neko.cef-custom-helper".to_string();
        ...
        eprintln!(
            "cef-custom-helper starting raw host url={} size={}x{} transparent={} fps={}",
        ...
        match CefCBridge::attach(handle.clone(), cef_config) {
            Ok(bridge) => {
                let loop_mode = format!("{:?}", bridge.message_loop_mode());

                eprintln!(
                    "cef-custom-helper attached backend={} loop_mode={} url={}",
                    bridge.backend(),
                    loop_mode,
                    bridge.config().url
                );
...
fn resolve_url() -> Result<String> {
    ...
    bail!("missing URL argument; pass a URL as argv[1] or set NEKO_CEF_HELPER_URL")
}
