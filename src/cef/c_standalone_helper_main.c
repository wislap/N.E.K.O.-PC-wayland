...
static void on_load_end(void* user_data, int http_status_code) {
  ...
  if (state && state->browser && http_status_code >= 200 && http_status_code < 400) {
    static const char* k_region_script =
        ...
        "'.neko-hosted-window','.neko-hosted-window-titlebar','.neko-hosted-window-close',"
        "'.neko-hosted-window-frame','.neko-hosted-window-resize','.el-overlay','.el-overlay-dialog',"
        "'.el-dialog','.el-drawer','.el-popper','.el-dropdown-menu','.el-select-dropdown',"
        "'.el-menu','.el-sub-menu','.el-menu-item','.el-popover','.el-tooltip__popper',"
        "'.el-message-box','.el-cascader__dropdown','[class*=\"popover\"]','[class*=\"sidepanel\"]',"
        "'[class*=\"sidebar\"]','[class*=\"dropdown\"]','[class*=\"submenu\"]','[class*=\"menu-item\"]',"
        "'[class*=\"popup-item\"]','[class*=\"toggle-item\"]'];"
        "const interactiveLeafSelector='button,input,textarea,select,summary,details,dialog,iframe,a[href],[role=\"button\"],[role=\"dialog\"],[role=\"menu\"],[role=\"menuitem\"],[role=\"tab\"],[role=\"switch\"],[role=\"checkbox\"],[role=\"radio\"],[role=\"slider\"],[role=\"textbox\"],[role=\"tooltip\"],[role=\"option\"],[role=\"listbox\"],[role=\"treeitem\"],[role=\"combobox\"],[role=\"link\"],[contenteditable=\"true\"],[tabindex],[aria-haspopup=\"true\"],[data-neko-interactive],[data-action],[onclick]';"
        "let last='';"
        "let lastDrag='';"
        "let lastNoDrag='';"
        ...
        "text.includes('agent')||text.includes('task')||text.includes('hosted')||"
        "text.includes('window')||text.includes('frame')||text.includes('plugin')||"
        "text.includes('dropdown')||text.includes('submenu')||text.includes('claw');"
        "}"
        ...
        "}"
        "function looksLikeMenuItem(el){"
        "if(!el)return false;"
        "const role=(el.getAttribute('role')||'').toLowerCase();"
        "const cls=(el.className||'').toLowerCase();"
        "return role==='menuitem'||role==='button'||role==='tab'||role==='option'||role==='treeitem'||role==='combobox'||"
        "cls.includes('menu-item')||cls.includes('popup-item')||cls.includes('dropdown-item')||cls.includes('submenu')||"
        "cls.includes('toggle-item')||cls.includes('button')||cls.includes('select-option');"
        "}"
        ...
        "}catch(_){}}"
        "function addSidePanelBridgeRects(rects){"
        "try{"
        "for(const panel of document.querySelectorAll('[data-neko-sidepanel]')){"
        "if(!panel||!visibleAndEnabled(panel)||!acceptsPointer(panel))continue;"
        "const anchor=panel._anchorElement;"
        "if(!anchor||!visibleAndEnabled(anchor)||!acceptsPointer(anchor))continue;"
        "const a=anchor.getBoundingClientRect();"
        "const p=panel.getBoundingClientRect();"
        "if(a.width<2||a.height<2||p.width<2||p.height<2)continue;"
        "const pad=4;"
        "if(p.left>=a.right-2){"
        "addRect(rects,{left:a.right-pad,top:Math.min(a.top,p.top)-pad,right:p.left+pad,bottom:Math.max(a.bottom,p.bottom)+pad});"
        "}else if(p.right<=a.left+2){"
        "addRect(rects,{left:p.right-pad,top:Math.min(a.top,p.top)-pad,right:a.left+pad,bottom:Math.max(a.bottom,p.bottom)+pad});"
        "}else if(p.top>=a.bottom-2){"
        "addRect(rects,{left:Math.min(a.left,p.left)-pad,top:a.bottom-pad,right:Math.max(a.right,p.right)+pad,bottom:p.top+pad});"
        "}else if(p.bottom<=a.top+2){"
        "addRect(rects,{left:Math.min(a.left,p.left)-pad,top:p.bottom-pad,right:Math.max(a.right,p.right)+pad,bottom:a.top+pad});"
        "}"
        "}"
        "}catch(_){}}"
        "function addIframeRects(rects,seen){"
        "try{"
        "for(const frame of document.querySelectorAll('iframe')){"
        "if(!frame||seen.has(frame)||!visibleAndEnabled(frame)||!acceptsPointer(frame))continue;"
        "seen.add(frame);"
        "addRect(rects,frame.getBoundingClientRect());"
        "}"
        "}catch(_){}}"
        "function addGenericClickableRects(rects,seen){"
        "for(const el of document.querySelectorAll('body *')){"
        "if(!el||seen.has(el)||!visibleAndEnabled(el)||!acceptsPointer(el))continue;"
        "const r=el.getBoundingClientRect();"
        "if(r.width<2||r.height<2)continue;"
        "if(r.width>=window.innerWidth*0.98&&r.height>=window.innerHeight*0.98&&!looksLikeInteractiveOverlay(el))continue;"
        "if(!(looksClickable(el)||looksLikeMenuItem(el)))continue;"
        "seen.add(el);"
        "addRect(rects,r);"
        "promoteInteractiveAncestors(rects,el,seen);"
        "}"
        "}"
        "function collect(){"
        "scheduled=false;"
        ...
        "}"
        "addIframeRects(uiRects,seen);"
        "addGenericClickableRects(uiRects,seen);"
        "addSidePanelBridgeRects(uiRects);"
        "const normalizedUi=normalizeRects(uiRects,384);"
        "const normalizedAvatar=normalizeRects(avatarRects,768);"
        "const mergedRects=normalizeRects(normalizedUi.concat(normalizedAvatar),1152);"
        "const payload=JSON.stringify(mergedRects);"
        "if(payload!==last){last=payload;console.log('NEKO_INPUT_REGION:'+payload);}"
        "const stats='rects='+mergedRects.length+' ui='+normalizedUi.length+' avatar='+normalizedAvatar.length+' iframes='+document.querySelectorAll('iframe').length+' hosted='+document.querySelectorAll('.neko-hosted-window').length+' sidepanels='+document.querySelectorAll('[data-neko-sidepanel]').length;"
        "if(stats!==lastStats){lastStats=stats;console.log('NEKO_INPUT_REGION_STATS:'+stats);}"
        ...
        "window.addEventListener('neko-sidepanel-layout-changed',function(){if(timer)clearTimeout(timer);scheduled=true;requestAnimationFrame(collect);},{passive:true});"
...
static void on_console(void* user_data,
  ...
  if (message && strncmp(message, "NEKO_INPUT_REGION_STATS:", 24) == 0) {
    emit_event("input_region_stats", message + 24);
    return;
  }
