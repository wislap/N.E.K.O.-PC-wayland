...
static void on_load_end(void* user_data, int http_status_code) {
  ...
  if (state && state->browser && http_status_code >= 200 && http_status_code < 400) {
    static const char* k_region_script =
        ...
        "'#live2d-return-button-container','#chat-container','#text-input-area',"
        "'.modal-dialog','.modal-overlay','[id^=\"live2d-popup-\"]','[id^=\"vrm-popup-\"]',"
        ...
        "addRect(rects,{left:Number(bounds.left),top:Number(bounds.top),right:Number(bounds.right),bottom:Number(bounds.bottom)});"
        "}catch(_){}}"
        ...
        "addRect(rects,{left:left,top:top,right:right,bottom:bottom});"
        "}catch(_){}}"
        "function collect(){"
        "scheduled=false;"
        "const rects=[];"
